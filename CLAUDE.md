...
| `machines/` | `machines.service.ts`, `machines.controller.ts` | CRUD + seed A/B/C + changerMode + arretUrgence + redemarrer + renommer |
...
| `common/initialisation/` | `initialisation.service.ts`, `machine-supprimee.schema.ts` | Seed machines A/B/C au démarrage + blacklist suppression + correction états incohérents |
...
**Synchronisation `etat` / `statut` des machines :**
- `arretUrgence()` → `etat: 'arretee'` + `statut: 'hors_ligne'` (DB + Socket.IO)
- `redemarrer()` → `etat: 'en_marche'` + `statut: 'en_ligne'` (DB + Socket.IO)
- Au démarrage : `InitialisationService` corrige les machines `{ etat: arretee, statut: en_ligne }` → `statut: hors_ligne` (état incohérent possible après crash ou arrêt forcé)
...
**Broker utilisé** : Mosquitto installé localement sur le PC de développement (service Windows).
- IP locale : `192.168.1.161` (IP du PC sur le réseau WiFi — peut changer selon le réseau)
- Port : `1883`
- Pas de compte cloud — Mosquitto tourne comme service Windows en local
- L'ESP32 se connecte à cette même IP pour publier ses données
- Si l'IP change : mettre à jour `MQTT_BROKER_URL` dans `backend-iot/.env`
- Sur serveur Linux : `sudo apt install mosquitto && systemctl enable mosquitto` → utiliser `mqtt://localhost:1883`

---

## Déploiement en production

### Analyse de préparation (mai 2026)

| Composant | Statut | Notes |
|-----------|--------|-------|
| Backend NestJS | OK | CORS, Helmet, Swagger conditionnel, ValidationPipe, fallbacks env |
| Frontend React | OK | URLs depuis env vars, intercepteur JWT, reconnexion Socket.IO |
| IA Python | OK | CORS configurable, host=0.0.0.0, fichiers .pkl présents (67 MB) |
| Tests (18 spec.ts) | OK | Mocks NestJS corrects, tous passent |
| Secrets `.env` | ATTENTION | JWT_SECRET trop court, MongoDB mot de passe faible |

### Actions obligatoires avant déploiement sur le serveur

**Secrets à regénérer** (ne jamais garder les valeurs de dev en prod) :

```bash
# Générer un JWT_SECRET fort (64+ caractères)
node -e "console.log(require('crypto').randomBytes(64).toString('hex'))"
```

- `JWT_SECRET` : remplacer `solution_embarquee_secret_2026` par 64+ caractères aléatoires
- MongoDB Atlas : changer le mot de passe `admin123` pour quelque chose de fort
- Email : regénérer le mot de passe d'application Google si nécessaire

### Checklist complète déploiement serveur Linux

**Secrets et configuration :**
- [ ] `JWT_SECRET` : 64 caractères aléatoires minimum (pas `solution_embarquee_secret_2026`)
- [ ] MongoDB Atlas : nouveau mot de passe fort (pas `admin123`)
- [ ] `NODE_ENV=production` → Swagger désactivé automatiquement
- [ ] `CORS_ORIGIN=http://IP_SERVEUR:3000` (ou domaine) dans `.env` backend
- [ ] `REACT_APP_API_URL=http://IP_SERVEUR:3001/api` dans `frontend-iot/.env.production`
- [ ] `REACT_APP_SOCKET_URL=http://IP_SERVEUR:3001` dans `frontend-iot/.env.production`
- [ ] `.env` bien absent du dépôt git (vérifié dans `.gitignore`)

**MQTT sur serveur Linux :**
- [ ] `sudo apt install mosquitto mosquitto-clients`
- [ ] `sudo systemctl enable mosquitto && sudo systemctl start mosquitto`
- [ ] `MQTT_BROKER_URL=mqtt://localhost:1883` dans `.env` backend (ou laisser vide si pas d'ESP32)

**Build et lancement :**
- [ ] `npm run build` dans `frontend-iot/` → génère `frontend-iot/build/`
- [ ] `npm run build` dans `backend-iot/` → génère `backend-iot/dist/`
- [ ] `pip install -r requirements.txt` dans `ia-iot/`

**Avec PM2 (recommandé) :**
- [ ] `npm install -g pm2` sur le serveur
- [ ] `pm2 start ecosystem.config.js` dans `backend-iot/`
...
# Backend
cd backend-iot && npm run build
pm2 start ecosystem.config.js   # ou : node dist/main.js

# IA
cd ia-iot && uvicorn main:app --host 0.0.0.0 --port 8000
```

### Servir le frontend (Nginx ou serve)
```bash
# Option 1 : Nginx (recommandé)
# Copier frontend-iot/build/ dans /var/www/html/ ou configurer un vhost

# Option 2 : serve (rapide pour démo)
npm install -g serve
serve -s frontend-iot/build -l 3000
```
